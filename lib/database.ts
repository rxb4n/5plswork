...
export async function createRoom(
  ...
  try {
    ...
    // Insert room with ON CONFLICT to handle duplicates gracefully
    await client.query(
      "INSERT INTO rooms (id, game_state, target_score, last_activity, created_at) VALUES ($1, $2, $3, NOW(), NOW()) ON CONFLICT (id) DO NOTHING",
    ...
    if (!room) {
      console.log(`Room ${roomId} could not be created (likely due to duplicate ID)`);
      return null;
    }
...
export async function updateRoom(
  roomId: string,
  updates: Partial<Omit<Room, "players">>
): Promise<boolean> {
  const client = await pool.connect();
  try {
    const updateFields: string[] = [];
...
export async function removePlayerFromRoom(playerId: string): Promise<boolean> {
  ...
  try {
    ...
    if (playerResult.rows.length === 0) return true;
...
export async function cleanupOldRooms(): Promise<void> {
  ...
  try {
    const result = await client.query(`
      ...
      RETURNING id
    `);
    console.log(`Cleaned up ${result.rowCount} old rooms: ${result.rows.map(r => r.id).join(", ")}`);
  } catch (error) {
